...
.courses_inner {
	...
	justify-content: center;
	gap: 1rem;
	border-bottom: 2px solid #342423;
	padding-bottom: 5rem;
	max-width: 95vw;
}
...
<body>
 ...
 %>
		...
		%>
		<!-- Xử lí thông báo thanh toán -->
		<%-- <script src="assets/ThongBao.js"></script>
		<%
		if (request.getAttribute("checkTT") != null) {
			boolean checkTT = (boolean) request.getAttribute("checkTT");
			if (checkTT == true) {
		%>
		<script type="text/javascript">
			createToast('success', 'fa-solid fa-circle-check', 'Thành Công',
	...
	<div class="courses_wrapper">
		...
		<div class="courses_qr" style="text-align: center;">
			...
			<div id="stck" style="display: none">
				<p style="font-weight: bold; font-size: 20px" >
				Số tiền: <span id="paid_price"></span>
				</p>
			</div>
		</div>
	</div>
	...
	<a id="httt" href="ThanhToanCKController?btnHTTT&ghichuGio=<%=magio%>" onclick="disableButton(this)"
		style="float:right;display:none;  padding: 15px 60px;font-size: 16px; font-weight:bold; background-color: red; color: #fff; border-radius: 5px; text-decoration: none; margin: 0 70px 20px 0">Hoàn
		tất thanh toán 
	</a>
